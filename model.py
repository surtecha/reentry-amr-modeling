...
merged_df = merged_df[(merged_df['amr'] >= 0.001) & (merged_df['amr'] <= 0.01)]
...
sequence_cols = ['semi_major_axis', 'bstar', 'eccentricity', 'mean_motion']
seq_length = 100
...
def build_model(seq_length, num_features):
    ...
    for i in range(num_features):
        # Create input for each feature sequence
        input_seq = Input(shape=(seq_length,), name=f'input_{i}')
        inputs.append(input_seq)
        
        # Properly reshape the input for LSTM using Keras layers
