...
def predict_amr(model, input_sequences, target_scaler, metadata):
    """Use the model to predict AMR value, using log10 transformation from the training process"""
    ...
    # Inverse transform the scaled prediction to get the log10 value
    prediction_log = target_scaler.inverse_transform(prediction_scaled)[0][0]
    
    # Inverse transform the log10 to get the actual AMR value
    # This matches the y_train_log = np.log10(y_train).reshape(-1, 1) transform from the training script
    prediction_amr = np.power(10, prediction_log)
    
    return prediction_amr
...
def main():
    ...
    try:
        ...
        # Predict AMR
        amr_prediction = predict_amr(model, input_sequences, target_scaler, metadata)
